...
U09已完成：101个容量—功率主网格、101个反序重跑和101个高投资稳健性组合全部Optimal，42项测试通过。冻结网格内唯一最低成本点为70 MWh / 25 MW；该结果仅对24小时构造算例与示例成本成立。详见docs/grid_search_analysis.md。U10尚未开始。

U08已完成：风光系数0.8/1.0/1.2/1.4与储能0/40/80 MWh共12场景，全部Optimal并通过验证和完整重跑；结果见docs/renewable_penetration_analysis.md。

U07独立数值审计通过但旧归因设计无效；U07R完成8组合及反序重跑，用Shapley分配三个约束的模型内影响。见docs/factorial_attribution_report.md。

## 最新进展：U06已完成

已实现带充放电互斥的MILP时序调度。7个场景全部通过；模型说明见docs/optimization_model.md，结果见results/optimization/U06-20260913-02。01失败记录保留，02修复CBC文本解舍入，未修改物理参数或验收门槛。下方第1—6阶段为历史规则模型说明。
