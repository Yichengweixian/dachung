...
U11R04仍blocked/invalid，未补跑或改判；全项目研究仍未完成。下一阶段需另立完整矩阵修订方案，先确认再冻结，不制作PPT。证据见results/annual/U11R05-v01/、U11R05-v01-independent-audit.json。

## 2026-09-24 U11R04只读诊断完成，U11R05待确认

本轮没有优化调用：21阶段LP重放与原保存文件逐字节一致；18个权重固定带无简单上下界冲突。上一解在重建MPS最大残差约2.6523e-10但非零，不能证明精确可行；CBC紧约束数值敏感性仍是未证实假设。诊断解析器首轮错误保留，修正版3项测试与诊断v02通过。U11R04仍invalid。
已提出U11R05单模型诊断：仅关闭presolve，保留所有模型系数、固定带和求解容差；最多2次独立调用，首个非Optimal即停。方案待确认、未冻结未运行，不恢复原9组矩阵，不做PPT。

## 2026-09-24 U11R04正式运行停止（invalid）

隔离历史源码复核已解决前置阻塞：365日基准与252个旧代表日解重审通过，没有关闭源码检查。按已批准方案冻结后新增权重模块、运行及审计入口；专项6项、全量62项测试通过。
U11R04-v01实际101次权重LP+36次调度MILP后，在N48/seed42主权重lex_035阶段遇CBC Infeasible；立即停止，未重试、未放宽容差。工程blocked、verdict=invalid。N12/seed42比例误差1.561595个百分点未达标，N24/seed42单组通过不能代替完整9组结论。只读部分审计313项哈希及36个日解通过，证据见results/annual/U11R04-v01-partial-audit.json；这不是研究通过。
所有失败模型、日志与部分结果保留；疑似紧约束数值问题尚未确定。后续若改变容差或方法须另建修订单元，不改本单元冻结规则。用户不需要PPT，项目研究尚未全部完成。

## 2026-09-24 U11R04已确认，冻结前审计阻塞

用户已明确同意方法与数值，不再等待方案审批。核验U11R03时严格源码漂移闸门报错：T4修改了scripts/check_materials_numbers.py；旧产物561项哈希通过，但本轮逐时物理重审未进入。U11R02另893项哈希通过。已保存results/annual/U11R04-preflight-v01/failure.json及独立哈希报告；没有修改旧manifest，没有冻结、实现或新求解，当前blocked/null。按plan组1先报告异常；后续建议隔离历史代码重审，不降低研究门槛、不重复要求确认原方案。

## 2026-09-24 U11R04研究草案（待确认）

用户明确继续研究、不做PPT。新增U11R04-energy-calibrated-weights五份草案：保持真实日集合，只在原簇天数0.5至2倍范围内校准权重，沿用同一N跨三种子的1个百分点/2%门槛。方法及新参数待用户确认；not_started/null，无freeze、无实现、无新求解。预算只是设计，不是结果；输入电量校准属于样本内拟合，不声称独立验证。项目研究尚未全部完成。
