...
- [ ] 0.2 确认 U06 supported、U07 独立数值审计 supported、U07R supported；确认评价函数与 U05 一致（同一函数对象）
...
>
> 1. 新建 `src/grid_search.py`：生成 101 个 (E,P) 组合，逐组合调用 `src/optimization_model.py` 求解 + `src/economic_model.py` 评价（复用函数，禁止复制逻辑），记录每个组合的求解状态。
...
> 请核对并逐项报告：1) 组合总数 101 且无重复；2) 全部组合求解状态清单（Optimal 数量）；3) 最优组合的约束检查与独立检查通过；4) 热力图数据与 summary.csv 一致；5) 分析文档没有把样本内最优写成全局最优或经济结论。失败即停止。
