...
3. 当前 U00—U09 已登记；新工作从路线图中的 U10 开始，仍须先冻结单元判据。
...
| 单元 | 内容 | 状态 |
|---|---|---|
| U00 | 版本基线与产物清单冻结 | complete / supported（2026-09-13 当前环境） |
| U01—U05 | 第 1—6 阶段已完成工作（构造数据/无储能/储能/容量敏感性/经济性） | 基线登记 |
| U06 | 时序优化模型（MILP，含充放电互斥） | complete / supported（02运行） |
| U07 | 优化模型独立验证与规则调度对照 | complete / invalid（数值验证supported，归因设计invalid） |
| U07R | 全组合与 Shapley 归因修订 | complete / supported |
| U08 | 新能源渗透率场景矩阵（4 系数 × 3 储能） | complete / supported |
| U09 | 储能容量-功率网格搜索（E×P） | complete / supported |
...
**汇报最小路线**：U00 → U06 → U07/U07R → U08 → U09，现已具备“优化模型 + 渗透率矩阵 + 容量功率网格寻优”阶段汇报材料。
**完整路线**：继续 U10/U11（真实数据与全年尺度）→ U12 → U13 → U14。
