...
若实际运行因年度光伏线性估算超过150 MW而停止，使用新增的[U11R01修订单元](studies/U11R01-solar-output-cap/question.md)及[可直接转交的执行提示词](prompts/05-u11-solar-cap-revision.md)。其范围仅为年度输入转换、参数、测试和记录，不启动全年调度或聚类。

该修订保留原始气象值，把150 MW明确为本简化模型的交流侧可用输出上限，并独立记录封顶损失。封顶次数以程序实际计算为准，“2小时”不是验收阈值。已有U10/U11冻结规则和失败结果不被覆盖；新单元完成不代表U11年度研究完成。

不同工作副本可能存在未提交的进度。执行前应核对实际运行副本的文件和记录，不能用本包中旧STATUS覆盖另一副本的最新进度。此次新增规格本身不代表已运行任何修订。
...
| U11R01 | 年度光伏输出上限修订；只生成并核验年度输入 | not_started（规格草案） |
