"""No-solve forensic replay of U11R04's failed lexicographic LP stage."""
import argparse
...
from src.energy_calibrated_weights import calibrate
...
def main():
    ...
    run = ROOT/'results/annual/U11R04-v01'
    ...
    a = np.array([[inputs.loc[inputs.cluster == i, col].sum() for i in range(48)] for col in channels])
    b = full[channels].sum().to_numpy()
    stage_names = ['primary', 'secondary']+[f'lex_{i:03d}' for i in sorted(range(48), key=lambda i: reps.date_UTC.iloc[i])]
    records = []
    ...
    with patch('src.energy_calibrated_weights.FullPrecisionCBC', RecordedSolution), \
    ...
    evidence = dict(findings=findings, model_matches=matches, in_memory_residuals=memory_residuals,
                    ...
                    source_sha256={p:sha(ROOT/p) for p in ['scripts/diagnose_u11r04_serialization.py', 'src/energy_calibrated_weights.py', 'src/cbc_full_precision.py']},
